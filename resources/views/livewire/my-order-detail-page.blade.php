<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
   ...
   <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mt-5">
      ...
      @php
         ...
         switch ($order->status) {
             case 'confirmed':
                 $order_status_bg = 'bg-blue-500';
                 break;
             case 'processing':
                 $order_status_bg = 'bg-orange-500';
                 break;
             case 'shipping':
                 $order_status_bg = 'bg-green-600';
                 break;
             case 'shipped':
                 $order_status_bg = 'bg-green-700';
                 break;
             case 'delivered':
                 $order_status_bg = 'bg-green-700';
                 break;
             case 'cancelled':
                 $order_status_bg = 'bg-red-600';
                 break;
             case 'refunded':
                 $order_status_bg = 'bg-blue-600';
                 break;
             default:
                 $order_status_bg = 'bg-blue-500';
         }
      ...
      >
         <div class="p-4 md:p-5 flex gap-x-4">
            ...
            <div class="grow">
               ...
               <div class="mt-1 flex items-center gap-x-2">
                  <span
                     class="{{ $order_status_bg }} py-1 px-3 rounded text-white shadow capitalize"
                  >
                     {{ $order->status }}
                  </span>
               </div>
      ...
      @php
         ...
         if ($order->payment_status == 'paid') {
             $payment_status = '<span
                                 class="bg-green-700 py-1 px-3 rounded text-white shadow capitalize"
                              >
                                 Paid
                              </span>';
         } elseif ($order->payment_status == 'pending') {
             $payment_status = '<span
                                 class="bg-blue-500 py-1 px-3 rounded text-white shadow capitalize"
                              >
                                 Pending
                              </span>';
         } elseif ($order->payment_status == 'failed') {
             $payment_status = '<span
                                 class="bg-red-500 py-1 px-3 rounded text-white shadow capitalize"
                              >
                                 Failed
                              </span>';
         }
   ...
   <div class="flex flex-col md:flex-row gap-4 mt-4">
      <div class="md:w-3/4">
         ...
         <div class="bg-white overflow-x-auto rounded-lg shadow-md p-6 mb-4">
            <h1 class="font-3xl font-bold text-slate-500 mb-3">Shipping Address
            </h1>
      ...
      <div class="md:w-1/4">
         <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="flex justify-between mb-2">
               ...
               <span
                  class="font-semibold">{{ Number::currency($item->order->grand_total, 'inr') }}</span>
            </div>
